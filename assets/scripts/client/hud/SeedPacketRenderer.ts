...
import type { FontConfig, FontLayerConfig } from '@/client/font/FontRenderer.d'
...
export class SeedPacketRenderer {
    static drawSeedPacket(args: SeedPacketRenderArgs): Node {
        ...
        const root = uiNode({
            name: args.name ?? 'SeedPacket',
            parent: args.parent,
            layer: args.layer,
            anchor: { x: 0, y: 1 },
            size: { width: SEED_PACKET_WIDTH * scale, height: SEED_PACKET_HEIGHT * scale },
    ...
    private static _drawBackground(root: Node, args: SeedPacketRenderArgs, scale: number) {
        if (scale > 1) {
            ...
            uiNode({
                ...
                size: { width: SEED_PACKET_WIDTH * scale, height: SEED_PACKET_HEIGHT * scale },
                anchor: { x: 0, y: 1 },
                sprite: { frame: args.seedPacketLarger, sizeMode: Sprite.SizeMode.RAW, trim: false },
            })
            return
        }
        ...
        uiNode({
            ...
            anchor: { x: 0, y: 1 },
            sprite: {
                frame: getAtlasFrame(args.seeds, backgroundCel, SEED_PACKET_WIDTH, SEED_PACKET_HEIGHT, SEED_PACKET_ATLAS_COLUMNS),
    ...
    private static _drawScaledCachedPlant(root: Node, seedId: number, args: SeedPacketRenderArgs, scale: number) {
        ...
        const plantRoot = uiNode({
            name: 'PacketPlant',
            parent: root,
            layer: args.layer,
            anchor: { x: 0, y: 1 },
            size: { width: PLANT_PREVIEW_CACHE_CELL_WIDTH, height: PLANT_PREVIEW_CACHE_CELL_HEIGHT },
            position: { x: placement.offsetX * scale, y: -(placement.offsetY + 1) * scale, z: 10 },
        }).node
        ...
        const icon = uiNode({
            ...
            parent: plantRoot,
            layer: args.layer,
            position: { x: PLANT_PREVIEW_CACHE_OFFSET_X, y: -PLANT_PREVIEW_CACHE_OFFSET_Y },
            size: { width: PLANT_PREVIEW_CACHE_CELL_WIDTH, height: PLANT_PREVIEW_CACHE_CELL_HEIGHT },
            anchor: { x: 0, y: 1 },
            sprite: {
                frame: getAtlasFrame(
    ...
    private static _drawCost(
        ...
        const node = uiNode({ name: 'Cost', parent: root, layer, anchor: { x: 0, y: 1 } }).node
    ...
    private static _drawScaledCostMatrix(
        ...
        const costRoot = uiNode({ name: 'Cost', parent: root, layer, anchor: { x: 0, y: 1 } }).node
        ...
        for (let charIndex = 0; charIndex < chars.length; charIndex++) {
            ...
            for (let layerIndex = 0; layerIndex < config.layers.length; layerIndex++) {
                ...
                const charNode = uiNode({
                    ...
                    size: { width: charInfo.rect[2] * scale, height: charInfo.rect[3] * scale },
                    anchor: { x: 0, y: 1 },
                    sprite: { frame: spriteFrame, sizeMode: Sprite.SizeMode.RAW, trim: false },
                }).node
