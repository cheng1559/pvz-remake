...
import type { BitmapFontAssets } from './BitmapFontAssets'
...
export class FontRenderer extends Component {
    ...
    private _rebuild() {
            ...
            for (let i = 0; i < resolvedChars.length; i++) {
                if (resolvedChars[i] === 10) {
                    lines.push({
                        ...
                        width: this._measureLineWidth(layer0, resolvedChars, lineStart, i, pointSize),
                    })
                    lineStart = i + 1
                    lineWidth = 0
                    prevChar = 0
        ...
        for (const cmd of commands) {
            ...
            const glyph = uiNode({
                name: `char_${cmd.charCode}_L${cmd.layerIdx}`,
                parent: this.node,
                position: { x: cmd.x, y: -cmd.y },
                size: { width: rect[2] * scale, height: rect[3] * scale },
                anchor: { x: 0, y: 1 },
                sprite: { frame: sf, sizeMode: Sprite.SizeMode.CUSTOM },
            })
            ...
            this._charNodes.push(glyph.node)
        ...
        if (selfTrans) {
            selfTrans.setContentSize(this._totalWidth, this._totalHeight)
        }
    }
